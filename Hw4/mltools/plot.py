...
def plotClassify2D(learner, X, Y, pre=lambda x: x, axis=None, nGrid=128, bgalpha=0.3, **kwargs):
    """
    ...
    if learner is not None:
        ...
        axis.imshow( YGrid.reshape( (len(xticks),len(yticks)) ), extent=ax, interpolation='nearest',origin='lower',alpha=bgalpha, aspect='auto' , vmin=vmin,vmax=vmax)
    else:
        vmin=vmin-.1; vmax=vmax+.1;
